...
import { AlertTriangle, Check, ArrowRight, Image as ImageIcon, X } from 'lucide-react';

interface DuplicateResult {
    filename: string;
    isDuplicate: boolean;
    existingPhotoId?: string | number;
            ...
            <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden">
                <div className="p-8">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-slate-900">נמצאו תמונות עם שם זהה ({duplicateCount})</h3>
                            <p className="text-slate-500 text-sm mt-1">מתוך {totalFiles} תמונות שבחרת, מצאנו {duplicateCount} שמות שכבר קיימים.</p>
                        </div>
                    </div>

                    <div className="space-y-2 mb-8 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                        {duplicates.slice(0, 5).map((dup, i) => (
                            <div key={i} className="flex items-center gap-3 p-2 bg-slate-50 rounded-xl border border-slate-100 text-right">
                                <div className="w-12 h-12 rounded-lg bg-slate-200 flex items-center justify-center overflow-hidden flex-shrink-0">
                                    {dup.existingThumbnailUrl ? (
                                        <img src={dup.existingThumbnailUrl} alt={dup.filename} className="w-full h-full object-cover" />
                                    ) : (
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        ...
                        >
                            <div>
                                ...
                                <p className="text-xs text-slate-500">העלה רק תמונות חדשות</p>
                            </div>
                        ...
                        >
                            <div>
                                <p className="font-bold text-slate-900 group-hover:text-slate-800">העלה הכל (גם וגם)</p>
                                <p className="text-xs text-slate-500">שמור את שתי הגרסאות</p>
                            </div>
